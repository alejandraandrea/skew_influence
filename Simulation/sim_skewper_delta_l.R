...
x1<-runif(N,0,1)
x1<-sort(x1)
beta1<-1 #initial value beta1 defined by fit skew-logistic model
eta<-beta1*x1

#Function to calculate the CDF of perturbed logistic 
pdf.sl<-function(z,lambda){2*dlogis(z)*pnorm(lambda*z)}
cdf.sl<-function(z,lambda){integrate(pdf.sl,lower=-Inf,upper=z,lambda=lambda,stop.on.error=FALSE)$value}

lambda<-0
print(lambda)
prob<-sapply(eta,cdf.sl,lambda) 
...
EPL<-matrix(NA,pl,R) #matrix of parameter estimates of logistic model
EPper<-matrix(NA,pl,R)
...
while(r<=R){
  ...
  df<-data.frame(y,x1)
  #estimation
  fit.l<-try(glm(y~x1-1,family=binomial(link="logit"),data=df),TRUE) #TRUE  the report of warnings is suppressed
  if(!inherits(fit.l,"try-error")){#If there is no error
    EPL[,r]<-fit.l$coefficients #to store the parameter estimates
    ...
    options(warn = 2) 
    fitper<-try(glm(Y[,r]~x1delta-1,family=binomial(link="logit"),data=dfdelta),TRUE)
...
for(r in 1:R){
  ...
  lambdaH0<-0
  lambda<-lambdaH0
  #betas<-matrix(EPL[,r],nrow=p,ncol=1,dimnames = NULL)
  ...
  k1<-8
  IPSE[,r]<-ipcncd(k1,q,cuttofSE,CNCL)
}
